# nf-core/updhmm: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/updhmm/usage](https://nf-co.re/updhmm/usage)

> _Documentation of pipeline parameters is generated automatically from the pipeline schema and can no longer be found in markdown files._

## Introduction

**nf-core/updhmm** is a pipeline for detecting uniparental disomy (UPD) events in trio sequencing data (proband, mother and father). The pipeline processes VCF or gVCF files to identify genomic regions where both chromosome copies are inherited from a single parent.

<!-- TODO nf-core: Add documentation about anything specific to running your pipeline. For general topics, please point to (and add to) the main nf-core website. -->

## Samplesheet input

You will need to create a samplesheet with information about the trios you would like to analyze before running the pipeline. Use this parameter to specify its location.
...
The samplesheet must be a **comma-separated file (CSV)** with the following 10 columns and a header row:
...
```bash
nextflow run nf-core/updhmm -profile docker -params-file params.yaml
```
...
First, go to the [nf-core/updhmm releases page](https://github.com/nf-core/updhmm/releases) and find the latest pipeline version - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`. Of course, you can switch to another version by changing the number after the `-r` flag.
